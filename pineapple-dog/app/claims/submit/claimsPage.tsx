...
export default function ClaimsClientPage(
  ...
  */
  ...
  const [triage_tier, setTriage_tier] = useState<number>(2); // Default to something not 1 to show the button
  ...
  async function voting() {
    try {
      console.log('Voting has been moved to the Community Dashboard at /claims');
      alert("Please navigate to the Community Dashboard at /claims to vote on this tier.")
    } catch (err) {
      console.error(err);
    }
  }
      ...
      <div className={s.modal}>
        <div className={s.claimsPage}>
          ...
          <div className={s.buttonBox}>
            ...
            {triage_tier !== 1 && (
              <button onClick={voting} className="button-solid">
                Vote
              </button>
            )}
          </div>
