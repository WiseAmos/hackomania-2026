...
interface ChallengeCardProps {
  data: Wager;
  onAction: (data: Wager) => void;
}

export function ChallengeCard({ data, onAction }: ChallengeCardProps) {
  const wager = data;

  const photoUrl = wager.imageUrl || "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?auto=format&fit=crop&q=80&w=800";
  const user = wager.player1 || { name: "Player 1", avatar: "P", handle: "@p1" };
  const title = wager.title;
  const timestamp = wager.timeRemaining || "No time limit";
  const stake = wager.totalStake ?? 0;
  const caption = wager.description;
  ...
  return (
    ...
    >
      {/* Visual Accent */}
      <div className="absolute top-0 left-0 w-1 md:w-full h-full md:h-1 opacity-50 z-10 bg-gradient-to-b md:bg-gradient-to-r from-[#FF4D4D] to-[#6366F1]" />
      ...
      <div className="w-28 sm:w-36 md:w-full aspect-square md:aspect-[4/3] bg-slate-900 overflow-hidden relative shrink-0">
        ...
        <div className="hidden md:flex absolute top-3 right-3 z-20 flex flex-col gap-1.5 items-end">
          ...
          <div className="flex items-center gap-1.5 bg-slate-950/70 backdrop-blur-md px-2 py-1 rounded border border-white/10 shadow-xl">
            ...
            <span className="text-[9px] font-black text-white">{wager.participants?.length ?? 2}</span>
          </div>
          <div className="flex items-center gap-1.5 bg-slate-950/70 backdrop-blur-md px-2 py-1 rounded border shadow-xl bg-[#6366F1]/90 border-[#6366F1]/20">
        ...
        <div className="flex flex-col flex-grow">
          <div className="flex items-center justify-between md:justify-start gap-2 md:gap-3 mb-2 md:mb-3.5 min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-7 h-7 md:w-8 md:h-8 rounded-full text-white bg-[#6366F1] text-[10px] md:text-xs font-bold flex items-center justify-center shrink-0 shadow-lg border border-white/10 overflow-hidden">
          ...
          <div className="flex md:hidden items-center gap-2 mb-2 overflow-x-auto no-scrollbar pb-0.5">
            ...
            <div className="flex items-center h-5 bg-slate-700/50 px-1.5 rounded border border-white/5 shrink-0 whitespace-nowrap gap-1">
              <Users className="w-2.5 h-2.5 text-slate-400" />
              <span className="text-[8px] font-bold text-white/70 leading-none">{wager.participants?.length || 2}</span>
            </div>
            <div className="flex items-center h-5 px-1.5 rounded border shrink-0 whitespace-nowrap bg-[#6366F1]/15 border-[#6366F1]/10">
              <span className="text-[8px] font-bold leading-none text-[#6366F1]">S${stake}</span>
            </div>
          </div>
